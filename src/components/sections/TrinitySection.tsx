...
export default function TrinitySection() {
  const apps = [
    ...
    },
    {
      ...
      leads: [
        { name: 'Adebayo Johnson', status: 'Ready to Buy', value: '₦5,000,000' },
    ...
    },
    {
      ...
      properties: [
        { name: 'Lagos Prime Land', price: '₦2.5M', roi: '18%', sold: 6, total: 10 },
        { name: 'Abuja Smart Suite', price: '₦4.2M', roi: '22%', sold: 11, total: 15 }
      ],
      color: 'brand-blue'
    }
  ]
            ...
            >
              ...
              <div className={`space-y-6 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                <div className="space-y-2">
                  ...
                  <p className={`text-base sm:text-lg md:text-xl font-semibold text-${app.color}`}>
                    {app.subtitle}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  {app.features.map((feature, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div className={`w-5 h-5 rounded-full bg-${app.color} flex items-center justify-center`}>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {app.stats.map((stat, i) => (
                        <div key={i} className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
                          <div className={`text-xl sm:text-2xl font-bold text-${app.color} mb-1`}>
                            {stat.value}
                          </div>
                          <div className="text-xs text-brand-gray mb-1">{stat.label}</div>
                  ...
                  {app.leads && (
                    <div className="space-y-3">
                      {app.leads.map((lead, i) => (
                        <div key={i} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-brand-green transition-colors">
                          <div>
                            <div className="font-semibold text-brand-dark">{lead.name}</div>
                            <div className="text-sm text-brand-gray">{lead.status}</div>
                          </div>
                          <div className="text-right">
                            <div className="font-bold text-brand-dark">{lead.value}</div>
                        ...
                        <div key={i} className="p-4 border border-gray-200 rounded-lg">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              ...
                              <div className="text-sm text-brand-gray">Price per fraction</div>
                            </div>
                            <div className="text-right">
                              <div className="font-bold text-brand-blue">{property.price}</div>
                              <div className="text-sm text-green-600">ROI: {property.roi}</div>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            {Array.from({ length: property.total }).map((_, idx) => (
                              <div
                                key={idx}
                                className={`flex-1 h-2 rounded ${
                                  idx < property.sold ? 'bg-brand-blue' : 'bg-gray-200'
                                }`}
                              />
                            ))}
                          </div>
